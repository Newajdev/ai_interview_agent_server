...
class LocalEvaluationProvider implements EvaluationProvider {
	async evaluate(input: Parameters<EvaluationProvider["evaluate"]>[0]): Promise<EvaluationResult> {
		const answers = input.messages.filter((message) => message.role === "CANDIDATE");
		...
		const communicationScore = Math.min(100, Math.round(45 + averageWords * 1.5));
		const technicalScore = Math.min(100, Math.round(40 + answers.length * 8));
		const problemSolvingScore = Math.min(100, Math.round((technicalScore + communicationScore) / 2));
		...
		return {
			...
			problemSolvingScore,
			overallScore,
			strengths: answers.length ? ["Completed the interview conversation."] : [],
			weaknesses: averageWords < 35 ? ["Answers could include more supporting detail."] : [],
			recommendations: ["Use specific examples and measurable outcomes in future answers."],
			summary: `${input.candidateName ?? "The candidate"} completed the interview with an overall score of ${overallScore}/100.`,
		};
	}
}

export const evaluationProvider: EvaluationProvider = new LocalEvaluationProvider();
...
export async function evaluateInterview(interviewId: string) {
	...
	if (!interview.messages.some((message) => message.role === "CANDIDATE")) {
		throw new Error("At least one candidate answer is required before evaluation.");
	}
