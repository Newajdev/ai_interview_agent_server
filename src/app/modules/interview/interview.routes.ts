...
interviewRouter.post("/", validate(createInterviewSchema), createInterviewController);
interviewRouter.post("/:id/start", startInterviewController);
interviewRouter.post("/:id/messages", validate(messageSchema), addMessageController);
interviewRouter.post("/:id/answer", validate(messageSchema.pick({ content: true })), answerController);
