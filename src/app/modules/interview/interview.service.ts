...
export async function createInterview(candidateId: string) {
	...
	return prisma.interview.create({ data: { candidateId }, include: interviewInclude });
}

export async function getInterview(id: string) {
	const interview = await prisma.interview.findUnique({ where: { id }, include: interviewInclude });
	return interview ? { ...interview, timer: getInterviewTimer(interview.startedAt) } : null;
}
...
export async function startInterview(id: string) {
	...
	if (interview.status !== "DRAFT") return { conflict: true as const, interview };
	...
	return prisma.$transaction(async (transaction) => {
		...
		await transaction.interviewMessage.create({ data: { interviewId: id, role: "AI", content: question } });
...
export async function addMessage(id: string, input: CreateMessageInput) {
	...
	if (getInterviewTimer(interview.startedAt).isExpired) return { expired: true as const };
	return { message: await prisma.interviewMessage.create({ data: { interviewId: id, ...input } }) };
}
...
export async function answerAndGenerateQuestion(id: string, content: string) {
	...
	const nextMessage = await prisma.interviewMessage.create({ data: { interviewId: id, role: "AI", content: question } });
